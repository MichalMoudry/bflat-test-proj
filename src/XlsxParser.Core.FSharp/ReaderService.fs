...
type ReaderService() =
    ...
    member this.ParseWorkbook<'T> (file: Stream) format sheetName range =
        if file.Length = 0 then Seq.empty<Row>
        else
            let columnNamesCount = GetColumnNames(typeof<'T>).Count - 1
            if columnNamesCount <= 0 then Seq.empty<Row>
            else
                ...
                seq {
                    for i in 0..numberOfRows - 1 do
                        yield Row([| for y in 0..columnNamesCount -> Cell(worksheet.Data.ElementAt(i).GetValue y) |])
                }
